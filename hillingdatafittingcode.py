...
### HILLING INFORMATION
#######################
csvfilename="hillingdatafull.csv"
graphsavename="hilling.eps"
csv_xlabel="Age (years)"
csv_ylabel="Length (millimeters)"
### Initial conditions for each model
t0logfrac=2 # choose the smallest value of the independent variable here
...
logfrac_params_guess=(0.5,-0.2,2.3,178)
yfrac_params_guess=(0.9,-5.6,0.5,200)
yfrac_logfrac_params_guess=(0.9,-1.7,0.5,200)
aticiguessparams=(2,0.6,0.1,0.1)
ctnguessparams=(300,1,0)
...
def zsoln(t,nu,r,a):
    if t==t0logfrac:
        ...
        if currentKey in zsoln_memo:
            ...
            sumresult=-r*a - (1.0-r)*sum([(gamma(t-k-nu)/(gamma(-nu)*gamma(t-k+1)))*zsoln(k,nu,r,a) for k in range(t0logfrac,int(t))])
...
def gompertz_logfrac(t,nu,a,r,y0):
    if t==t0logfrac:
        return y0
    if t>t0logfrac:
        return y0*np.prod([1/(1-p(k,nu,r,a)) for k in range(t0logfrac,int(math.floor(t))+1)])
...
def gompertz_yfrac(t,nu,a,r,y0):
    ...
    if t>t0yfrac+1:
        ...
        if currentKey in gompertz_yfrac_memo:
            ...
            denomint=sum((1/gompertz_yfrac(tau,nu,a,r,y0))*sum(hnu(tau,s-1,-nu-1)*gompertz_yfrac(s,nu,a,r,y0) for s in range(t0yfrac+1,tau+1)) for tau in range(int(t0yfrac+1),int(t)))
...
def gompertz_yfrac_logfrac(t,nu,a,r,y0):
    ...
    if t>t0yfraclogfrac+1:
        ...
        if currentKey in gompertz_yfrac_logfrac_memo:
            ...
            numerator=(ominusr-1)*numeratorint
            denominatorint=sum((1/gompertz_yfrac_logfrac(tau,nu,a,r,y0))*hnu(t,tau-1,nu-1)*sum(hnu(tau,s-1,-nu-1)*gompertz_yfrac_logfrac(s,nu,a,r,y0) for s in range(int(t0yfraclogfrac+1),int(tau+1))) for tau in range(int(t0yfraclogfrac+1),int(t)))
...
#############################################
# Define continuous Hilling Gompertz function
#############################################
def hillingctngompertz(t,linf,G,t0):
    return linf*np.exp((-1)*np.exp(-G*(t-t0)))

def hillingctngompertz_vec_self(t,linf,G,t0):
    y=np.zeros(t.shape)
    for i in range(len(y)):
        y[i]=hillingctngompertz(t[i],linf,G,t0)
    return y
#######################################################
#######################################################
#######################################################
...
# Plot data
###########
plt.scatter(xdata,ydata,color='black',s=5,label='data')
...
###############
#################################
# Continuous Hilling Gompertz fit 
#################################
ctn_popt, ctn_pcov = so.curve_fit(hillingctngompertz_vec_self, xdata, ydata,p0=ctnguessparams)
plt.plot(xdata,[hillingctngompertz(x,*ctn_popt) for x in xdata],label="ctn",color="cyan")
...
# RSS, SE, R^2 for ctn
######################
ctn_RSS=sum([(ydata[int(k)-1]-hillingctngompertz(k,*ctn_popt))**2 for k in xdata])
